...
class Evaluator():
    ...
    def analysis_2_per_dg_model(self, dg_model_real: DGModel):
    ...
    def analysis_3_all_dg_models(self, dg_model_real: DGModel, n_learning: int = 100):
        corr_dict = {}
    ...
    def get_corr(self, dg_model):
        ...
        print(dg_model.name, data.X.head(5))
    ...
    def analysis_4_per_dg_model_var1(self, dg_model_real: DGModel, n_samples: int, tr_frac: float,
                                     n_reps: int):
        scores_per_dg_model = {}  # dg_model.name: None for dg_model in self.dg_models
    ...
    def analysis_4_per_dg_model_var2(self, dg_model_real: DGModel, n_samples: int, tr_frac: float,
                                     n_reps: int):
        all_scores = {dg_model.name: {ml_model.name: {score.__name__: [] for score in self.scores} for ml_model in
                                      self.ml_models} for dg_model in [dg_model_real, *self.dg_models]}

        for rep in range(n_reps):
            train_data, test_data = self.get_train_and_test_from_dg(dg_model_real, n_samples, tr_frac)

            for ml_model in self.ml_models:
                scores = self.develop_ml_model(ml_model, train_data, test_data)
                for score_name in scores.keys():
                    all_scores[dg_model_real.name][ml_model.name][score_name].append(scores[score_name])

            for dg_model in self.dg_models:
                dg_model.fit(train_data)
                if len(dg_model.model.nodes) != len(test_data.all.columns):
                    continue
                # here n_samples could be a different number
                learnt_train_data = dg_model.generate(n_samples, self.outcome_name)

                for ml_model in self.ml_models:
                    scores = self.develop_ml_model(ml_model, learnt_train_data, test_data)
                    for score_name in scores.keys():
                        all_scores[dg_model.name][ml_model.name][score_name].append(scores[score_name])
